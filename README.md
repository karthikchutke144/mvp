...
- Write the final predictions to the provided `dataset/output.csv` template
...
Install the Python dependencies first with:

```bash
python3 -m pip install -r requirements.txt
```

After running your solution, confirm that `dataset/output.csv` contains the required columns and one row for every request.

## Important File Locations

```text
dataset/        Input data and the output template. Do not modify the input data.
code/           Your solution code.
code.zip        ZIP file containing your complete solution for submission.
```

The template at `dataset/output.csv` is the final prediction file for this repository.
